...
export function SSDatePicker({ id = 'date', value, onChange, placeholder = 'Select date' }: PickerProps) {
  ...
  return (
    <View>
      <TouchableOpacity onPress={() => setOpenPickerId(id)} activeOpacity={0.9}>
        <Input value={value} placeholder={placeholder} editable={false} pointerEvents="none" />
      </TouchableOpacity>
      {show && (
        <Modal transparent animationType="slide">
          <TouchableOpacity className="flex-1 justify-center items-center bg-black/30" activeOpacity={1} onPress={() => setOpenPickerId(null)}>
            <TouchableOpacity activeOpacity={1} className="bg-white rounded-xl overflow-hidden">
              <DateTimePicker
                mode="date"
              ...
              />
              <TouchableOpacity className="p-4 bg-emerald-600 items-center" onPress={() => {
