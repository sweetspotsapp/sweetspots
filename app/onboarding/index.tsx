...
const QuestionTitle = ({ emoji, text }: { emoji: string; text: string }) => (
  <View className="mb-5">
...
const travelerTypeOpts = [
  { key: "planner", label: "I’m Planner – love organizing everything", emoji: "📒" },
...
const reqOpts = [
  ...
  { key: "familyFriendly", label: "Family / kid-friendly options", emoji: "🧸" },
...
export default function QuestionnaireScreen() {
  const { answers, ui, updateAnswers, nextStep, prevStep, dismiss, markCompleted, isComplete } =
    useOnboardingStore();
...
function renderStep(uiStep: number, answers: any, updateAnswers: any) {
  switch (uiStep) {
    case 0:
      return (
        <View>
          <QuestionTitle emoji="🧭" text="What kind of traveler are you?" />
          {renderOptions(
            [
              { key: "planner", label: "I’m Planner – love organizing everything", emoji: "📒" },
              { key: "chill", label: "I’m Chill – go with the flow", emoji: "🧘" },
    ...
    case 1:
      return (
        <View>
          <QuestionTitle emoji="☘️" text="How do you usually travel?" />
          {renderOptions(
            [
              { key: "solo", label: "Solo Explorer", emoji: "🧭" },
              { key: "friends", label: "Friend Group Adventurer", emoji: "🧑‍🤝‍🧑" },
    ...
    case 2:
      return (
        <View>
          <QuestionTitle emoji="🤙" text="What vibe are you chasing?" />
          {renderOptions(
            [
              { key: "chill", label: "Chill & laid-back", emoji: "🧊" },
              { key: "social", label: "Lively & social", emoji: "🎉" },
        ...
        <View>
          ...
          {renderOptions(
            [
              { key: "accessible", label: "Disability-accessible", emoji: "🧩" },
              { key: "dietary", label: "Dietary needs (vegan, halal etc.)", emoji: "🥗" },
              { key: "lowEnergy", label: "Low-energy / relaxing activities only", emoji: "🌙" },
              { key: "familyFriendly", label: "Family / kid-friendly options", emoji: "🧸" },
              { key: "smokeFree", label: "Smoke-free environment", emoji: "🚭" },
              { key: "petFriendly", label: "Pet-friendly spots", emoji: "🐾" },
              { key: "none", label: "None — I’m all good", emoji: "✈️" },
            ...
            (key: string) => {
              if (key === "none") return updateAnswers({ requirements: ["none"] });
              const set = new Set((answers.requirements as string[]).filter((r) => r !== "none"));
              set.has(key) ? set.delete(key) : set.add(key);
    ...
    <SSContainer>
      ...
      <View className="flex-row items-center justify-between py-3">
        <TouchableOpacity
          ...
          onPress={prevStep}
        >
          <ArrowLeft size={22} className={ui.step === 0 ? "text-slate-300" : "text-orange-500"} />
        </TouchableOpacity>
      ...
      {/* Step Content */}
      <View className="mb-6">{renderStep(ui.step, answers, updateAnswers)}</View>
