...
export default function PlaceSuggestionsPage() {
  ...
  const selectedItineraryPlace = itineraryPlaces.find((ip) => ip.placeId === selectedPlace?.id);
  const selectedItineraryPlaceId = selectedItineraryPlace ? String(selectedItineraryPlace.id) : null;
  ...
  return (
    <SSContainer>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View className="mb-3">
            <PlaceSuggestionCard itineraryPlace={item} onSelect={setSelectedPlace}/>
          </View>
        )}
